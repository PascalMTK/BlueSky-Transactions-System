...
<div class="stats-grid">
    ...
    <div class="stat-card red animate-on-scroll">
        ...
        <div class="stat-info">
            ...
            <div class="stat-change">{{ now()->locale('fr')->monthName }} {{ now()->year }}</div>
        </div>
    </div>
</div>
...
<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-bottom:20px;">
    ...
    {{-- Aujourd'hui --}}
    <div class="card animate-on-scroll" style="padding:16px;">
        <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:var(--text-muted);margin-bottom:12px;">☀️ {{ __('app.today') }}</div>
                ...
                @forelse($recentTransactions as $tx)
                    <tr>
                        ...
                        <td>
                            ...
                            <div style="font-size:11px; color:var(--text-muted)">+{{ number_format($tx->fee_amount, 0) }} fees</div>
                        </td>
                        <td style="color:var(--text-muted); font-size:12px">{{ $tx->created_at->format('d/m H:i') }}</td>
                    </tr>
                @empty
                    <tr><td colspan="5" style="text-align:center; padding:30px; color:var(--text-muted)">No transactions</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
        ...
        @forelse($topAgents as $index => $agent)
            <div style="padding:12px 18px; border-bottom:1px solid var(--divider); display:flex; align-items:center; gap:10px; transition:background 0.15s;" onmouseenter="this.style.background='var(--bg-row-hover)'" onmouseleave="this.style.background=''">
                ...
                <div style="flex:1; min-width:0">
                    ...
                    <div style="font-size:11px; color:var(--text-muted)">{{ $agent->country?->flag_emoji }} — {{ number_format($agent->tx_count ?? 0) }} tx</div>
                </div>
                <div class="amount-display amount-primary" style="font-size:12px; flex-shrink:0;">{{ number_format($agent->tx_amount ?? 0, 0, ',', ' ') }}</div>
            </div>
        @empty
            <div style="padding:30px; text-align:center; color:var(--text-muted)">No agents found</div>
        @endforelse
    </div>
</div>
...
const cLabels = countryStats.map(c => c.flag_emoji + ' ' + c.code);
const cData   = countryStats.map(c => parseFloat(c.sent_amount) || 0);
...
window.blueskyCharts = [monthlyChart, countryChart];

// Update charts on theme change
const observer = new MutationObserver(() => {
    [monthlyChart, countryChart].forEach(c => {
