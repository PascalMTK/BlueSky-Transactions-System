...
class AgentController extends Controller
{
    public function dashboard()
    {
        ...
        $totalAmt   = $base()->where('status', 'completed')->sum('amount');
        $totalFees  = $base()->where('status', 'completed')->sum('fee_amount');
        ...
        $stats = [
            ...
            // Deep stats (stats panel)
            'completion_rate'   => $total > 0 ? round($completed / $total * 100, 1) : 0,
            'avg_amount'        => $completed > 0 ? round($totalAmt / $completed, 2) : 0,
            'avg_fee'           => $completed > 0 ? round($totalFees / $completed, 2) : 0,
            'week'              => $base()->whereBetween('created_at', [now()->startOfWeek(), now()->endOfWeek()])->count(),
            'week_amount'       => $base()->whereBetween('created_at', [now()->startOfWeek(), now()->endOfWeek()])->where('status', 'completed')->sum('amount'),
            'send_count'        => $base()->where('transaction_type', 'send')->count(),
            'withdrawal_count'  => $base()->where('transaction_type', 'withdrawal')->count(),
        ];
